...
|----|------|:-------:|:-------:|:----:|------------|
| PRE-M01 | 前提 | ✓ | CI `pytest` | | |
| PRE-M02 | 前提 | | `test_cli` 等 | | |
| PRE-M03 | 前提 | | `test_init` | | |
| SERVE-M01 | Bridge 常駐 | ✓ | — | | `sayane serve` + curl（[manual-only §3.1](acceptance-manual-only.md)） |
| CLI-M01 | CLI | | `test_cli.py` | | |
| CLI-M02 | CLI | | `test_cli.py` | | |
| CLI-M03 | CLI | | `test_cli.py` | | |
| STOR-M01 | Storage | | `test_storage_backend.py` | | |
| STOR-M02 | Storage | ✓ | entries 件数は L1 | | メッセージ目視 |
...
| pytest 結果 | passed / skipped（実測値を記入） |
| L2 実施日 | YYYY-MM-DD（実施日を記入） |
| L3 実施日（該当時） | YYYY-MM-DD または N/A |
| 判定 | ☑ Pass / ☐ Fail |
