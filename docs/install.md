...
| **Windows（Commercial）** | **署名付き MSI** | [sayane-pro](https://github.com/zyx-corporation/sayane-pro) |
...
**Commercial Edition**（[sayane-pro](https://github.com/zyx-corporation/sayane-pro)）の Windows 向け第一配布方式は **署名付き MSI** である。詳細は [商用版ドキュメント §5](https://github.com/zyx-corporation/sayane-pro/blob/main/docs/commercial-edition.md) を参照。

| 項目 | 内容 |
|------|------|
| 形式 | x64 MSI（Authenticode 署名） |
| インストール | ユーザー単位（管理者不要） |
| データ | アンインストール時も `~/.sayane/` は保持 |
