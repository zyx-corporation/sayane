...
**現行バージョン**: Community Edition **v1.0.3**（PyPI: `pip install sayane`）
...
| 採用・拒否の履歴を残す | **lineage** |
| Markdown 文脈を Profile に載せる | `context/` / storage index / Obsidian・Git 運用 |
| エディタや外部ツールから使う | CLI / Local Bridge / MCP Server / Chrome Extension |
...
Community Edition **v1.0.3** では、以下が利用可能です。
...
| Storage | 利用可能 | Markdown / Obsidian / Git 指向の文脈運用 |
...
- PyPI で入るのは CLI + Bridge + MCP です。Chrome Extension は別途ビルドが必要です
