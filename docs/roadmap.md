...
| P6 | 進行中 | `/app/ui*` legacy retirement — maintainer/debug compatibility surface を通常の macOS operator flow から段階的に退避する |
...
- 直近は native macOS app の完成と `/app/ui*` maintainer/debug compatibility surface の縮退を優先する
...
- native diagnostics / reconnect / startup guidance は current macOS flow で routine 化済み
- native install/update/reinstall 導線は repo-local bundle path と release-zip path の両方を記録済み
- routine UI、handoff/export、launcher scripts から browser auto-open / Chrome-first wording は段階的に除去済み
- historical Bridge-hosted shell documents は current native-first flow から切り離して記録済み
- ADR 0029 により server-rendered `/app/ui` HTML は removable presentation layer として扱う方針を確定済み

M12 の残り:

- `docs/bridge-manual.md` と smoke/debug runbook 群に残る `/app/ui*` の maintainer/debug boundary をさらに明確化する
- ADR 0029 の retire 条件に沿って HTML rendering 固有依存を runbook / smoke から剥がす
- ADR 0028 に合わせて maintainer/debug seam と operator-facing `/app/*` contract の境界を release closure に反映する
