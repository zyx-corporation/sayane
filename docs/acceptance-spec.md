...
| 備考 | `ruff check src tests` pass · GitHub Release `v1.0.1`（本リリース） |
...
| sayane-pro **併用**開発環境で pytest | context_index / storage index 系が **RecursionError** — Community 判定は **pro なし venv** を正とする。pro 側フック修正は別 Issue |
