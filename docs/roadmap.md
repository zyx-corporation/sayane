...
**マイルストーン注記（Community 1.0）**: Phase 0〜5 の成果物をもって、Sayane Community Edition の `v1.0.0` リリース基準とする。

## 9. Phase 6: Commercial Edition — Rust 暗号化 + SQLite

Phase 6 は **Sayane Commercial Edition** のストレージ・性能層として位置づける。利用者向けマニュアル・実装詳細は sayane-pro 側で管理する（本節は OSS ロードマップ上の位置づけのみ）。
...
Community Edition（Apache 2.0）は FileSystem + Git + Python Core を **継続メンテ** する。Rust/SQLite backend・移行 CLI・MSI の実装本体は Commercial Edition 側とし、Storage 抽象の契約のみ OSS に残す（[storage-backend.md](storage-backend.md)）。
