...
Community Edition（Apache 2.0）は FileSystem + Git + Python Core + security abstraction contracts を継続メンテする。Rust/SQLite backend・MSI・商用UI拡張の実装本体は Commercial / Pro 側とし、Storage / Vault / Keychain 抽象の契約は OSS に残す。
...
3. Local Vault key management ADR の実装準備
4. PlatformKeychainProvider 抽象
5. SQLite persistent store abstraction
6. load_review_decisions() persistent implementation
7. Resident App Service / daemon
8. Candidate Review Queue UI
9. MCP compiled_context preview UI
10. Commercial / Pro packaging and performance work
```
