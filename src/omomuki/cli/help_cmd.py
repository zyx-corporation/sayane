"""Hierarchical help for the Omomuki CLI."""
...
import omomuki
from omomuki.cli.i18n import t


def register_help(app: typer.Typer) -> None:
    """Register top-level ``omomuki help [TOPIC...]``."""
...
def render_help(topics: list[str] | None) -> str:
    ...
    ctx = click.Context(root, info_name="omomuki")
    path = topics or []
...
def _format_overview(group: click.Group, ctx: click.Context) -> str:
    lines = [
        t("help.tagline", version=omomuki.__version__),
        "",
        t("help.usage_title"),
        t("help.usage_help"),
        t("help.usage_group"),
