...
| CLI-07 | `candidate list\|show\|evaluate\|diff\|approve\|reject\|lineage` | ✓ | 一部 `test_candidate_cli.py` |
| CLI-08 | `storage backend status\|list\|set filesystem` | ✓ | `test_storage_backend.py` |
| CLI-09 | `storage import` / `index` | ✓ | `test_storage_cli.py` |
| CLI-10 | `storage export` / `commit` | | 手動 |
| CLI-11 | `mcp list-profiles\|inspect-profile\|compile` | ✓ | `test_mcp_cli.py` |
| CLI-12 | `--lang ja` / `SAYANE_LANG` | | `test_cli_i18n.py` |
| CLI-13 | 未知 `--target` / Profile 不在 | ✓ | 手動 |
...
| `tests/test_cli_plugins.py` | PLG-01 |
