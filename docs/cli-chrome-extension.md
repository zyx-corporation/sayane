...
Chrome Extension は入口と出口である。Sayane の実行基盤において **LLM チャット UI と同格の実行要素** である（[ui-design-principles.md](ui-design-principles.md)）。
...
**基本コンセプト**（処理中 `wait` / 操作不可 `not-allowed`、状態の伝達）: [ui-design-principles.md](ui-design-principles.md)。
