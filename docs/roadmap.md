...
このPhaseでは、Local Bridge、Chrome Extension、MCP Server、RDE/UIB自動評価は実装しない。Sayaneの最小価値である `Profile → Prompt IR → Adapter` を動かすことに集中する。
...
Chrome Extensionは便利なUIであるが、長期的な安定接続面ではない。MCP Serverは、Cline、Cursor、Claude Desktop、その他MCP対応クライアントからSayaneのProfile / Context / Prompt IR生成機能を呼び出すための接続面となる。
...
目的は、ブラウザ上の文脈をSayaneへ取り込み、LLM入力欄へ挿入できること。

Chrome Extension は補助UIであり、Sayaneの本体ではない。DOM依存箇所は分離し、壊れてもCore / CLI / MCPが影響を受けない構成にする。
...
Phase 5 拡張（0.5.8）: 環境変数 `SAYANE_OBSIDIAN_VAULT` により `storage import` / `export` の vault 引数を省略可能。
...
Phase 6 は **Sayane Commercial Edition** のストレージ・性能層として位置づける。詳細: [sayane-pro](https://github.com/zyx-corporation/sayane-pro/blob/main/docs/commercial-edition.md)。
...
Community Edition（Apache 2.0）は FileSystem + Git + Python Core を **継続メンテ** する。Rust/SQLite backend・移行 CLI・MSI の実装本体は [sayane-pro](https://github.com/zyx-corporation/sayane-pro) とし、Storage 抽象の契約のみ OSS に残す。Rust 境界の詳細は [sayane-pro rust-boundary.md](https://github.com/zyx-corporation/sayane-pro/blob/main/docs/rust-boundary.md) を参照。
...
Sayaneはまず、local-firstなPersona / Context IR基盤として最小実装を完成させる。
