...
| CLI-04 | `sayane candidate list|evaluate|diff|approve|reject|lineage` | `tests/test_candidate_cli.py`, `tests/test_critical_merge.py` |
| CLI-05 | `sayane storage import|index|commit|backend status|list|set` | `tests/test_storage_cli.py`, `tests/test_storage_backend.py` |
| CLI-06 | `sayane mcp serve` および MCP サブコマンド | `tests/test_mcp_cli.py` |
...
| CLI-09 | `sayane-pro` 未インストール時、plugin hook は no-op | `tests/test_cli_plugins.py` |
...
| ID | 手順 | 期待結果 |
|----|------|----------|
| CLI-M01 | `sayane profile inspect` | Profile メタデータ表示 |
| CLI-M02 | `sayane compile --target chatgpt` | JSON（`messages` 含む） |
| CLI-M03 | `sayane compile --target claude` | JSON（Anthropic 形式） |
...
| CLI | `test_cli.py`, `test_cli_help.py`, `test_candidate_cli.py`, `test_storage_cli.py`, `test_mcp_cli.py` | CLI-01〜06 |
...
| MCP | `test_mcp_operations.py` | MCP-01〜04 |
| Plugins | `test_cli_plugins.py` | CLI-09 |
| i18n | `test_cli_i18n.py`, `test_cli_locale_resolve.py` | CLI-08 |
...
| 日付 | 版 | 内容 |
|------|-----|------|
| 2026-05-24 | 0.6.0 | 初版 — OSS Community 受け入れ仕様・L1/L2/L3 三層・契約対応表 |
