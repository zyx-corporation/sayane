...
**実行プロンプト（論文 T-RDE v1.2 準拠）**: [t-rde-execution-prompt.md](t-rde-execution-prompt.md)（v1.1a — 共鳴条件の同時充足、σ の暫定ヒューリスティック、非懲罰的監査）
...
**正規語彙（canonical_terms）のテスト**では、特定ユーザー・理論に依存しない中立フィクスチャ（例: `ABC` / `Approved Base Concept`）を用いる。Core に理論固有の略語をハードコードしない。受け入れ観点とフィクスチャ一覧は [canonical-terminology.md](canonical-terminology.md) を正とする。
...
PromptIR → GeminiAdapter → gemini_working_memo
PromptIR → DeepSeekAdapter → deepseek_working_memo
```

正規語彙の解決は Core（`canonical_terms.py`）が行い、Adapter は LLM 向け整形のみを担う。
...
Chrome Extension および将来の Web UI では、UI を **実行基盤の実行要素**（LLM チャット画面と同格）として扱う。おろそかにすれば入出力の意味喪失に至る。

- **処理中（busy）** → カーソル `wait`、進行ラベル、`aria-busy`
- **操作不可（unavailable）** → カーソル `not-allowed`、理由をテキストで示す
- 両者を `disabled` だけで混同しない

詳細・実装契約: [ui-design-principles.md](ui-design-principles.md)。実装: `extension/src/busy-ui.ts`。
...
Chrome ExtensionではDOM変更に弱い箇所を分離し、Coreへ判断を持ち込まない。対話 UI は [ui-design-principles.md](ui-design-principles.md) に従い、busy（処理中）と unavailable（操作不可）を区別して伝える。
...
**UI も例外ではない。** LLM チャットと同様、UI の設計劣化は入出力の意味喪失に直結する（[ui-design-principles.md](ui-design-principles.md)）。
