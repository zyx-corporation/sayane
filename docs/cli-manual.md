...
| `sayane init` | ローカル Profile Store を初期化する |
...
`--profile` を省略したコマンドは、既定で `~/.sayane/profiles/default/sayane.profile.yaml` を読み込む。
...
- `~/.sayane/profiles/default/` と `context/` を作成する
...
| オプション | 既定値 | 説明 |
|-----------|--------|------|
| `--profile` | `~/.sayane/.../sayane.profile.yaml` | 読み込む Profile YAML のパス |
...
sayane compile --target claude \
  --profile ~/.sayane/profiles/default/sayane.profile.yaml \
...
# ターミナル 2 — Extension ビルド後、Options に ~/.sayane/bridge.token を設定
...
| Bridge `401` | token 未設定・不一致 | `~/.sayane/bridge.token` と Authorization ヘッダ |
