...
Sayane は、CLI で最小価値を実証した後、Local Bridge、MCP Server、常駐ローカルアプリ、Local Vault、UI 統合へ進む。
...
Phase 5 は、セキュリティ境界が未確定な外部 vault / Git 連携へ広げず、FileSystem storage の最小維持に限定する。Obsidian import/export、Git 自動コミット、外部同期前提の長期保存導線は、Local Vault / 暗号化 / 鍵管理 / retention policy が固まるまで保留する。
...
## 8. Phase 5: FileSystem Storage Only / External Storage Hold

目的は、ProfileとContextを最小限の FileSystem storage として維持し、Local Vault 実装前に外部連携型ストレージへ拡張しないことである。

Obsidian vault import/export、Git commit integration、Git 自動コミット、外部同期前提の運用は保留する。理由は、local storage の暗号化、鍵管理、retention policy、Deep Private / Layer 3 の扱い、外部同期時の漏洩リスクがまだ解決されていないためである。
...
**マイルストーン注記（Community 1.0）**: Phase 0〜5 の成果物は、FileSystem storage を前提とした最小 Community workflow の基準とする。Obsidian / Git integration は Community 1.0 の必須条件から外す。
...
3. Phase 5 を FileSystem storage のみに縮退
4. Local Vault key management ADR の実装準備
5. PlatformKeychainProvider 抽象
6. SQLite persistent store abstraction
7. load_review_decisions() persistent implementation
8. Resident App Service / daemon
9. Candidate Review Queue UI
10. MCP compiled_context preview UI
11. Obsidian / Git integration の再評価
12. Commercial / Pro packaging and performance work
```
