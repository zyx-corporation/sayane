"""Storage backend configuration (~/.sayane/config.yaml)."""
...
def config_path(home: Path | None = None) -> Path:
    return (home or sayane_home()) / CONFIG_FILENAME
...
def save_storage_config(
    ...
    *,
    ...
    """Persist storage config to ~/.sayane/config.yaml."""
    path = config_path(home)
...
def profile_dir_for_id(profile_id: str, home: Path | None = None) -> Path:
    return (home or sayane_home()) / "profiles" / profile_id
