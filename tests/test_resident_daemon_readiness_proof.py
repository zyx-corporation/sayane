"""Tests for resident daemon readiness-proof preview."""
...
def test_readiness_proof_reports_stopped_daemon_as_not_ready(tmp_path: Path) -> None:
    runtime_root = tmp_path / "run"

    payload = build_readiness_proof(runtime_root).public_metadata()

    assert payload["kind"] == "resident_daemon_readiness_proof_preview"
    assert payload["preview_only"] is True
    assert payload["readiness_status"] == ResidentDaemonReadinessProofStatus.READINESS_NOT_READY.value
...
def test_readiness_proof_reports_manual_review_when_status_is_ambiguous(
    tmp_path: Path,
) -> None:
    runtime_root = tmp_path / "run"
    ...
    assert payload["readiness_status"] == ResidentDaemonReadinessProofStatus.MANUAL_REVIEW_REQUIRED.value
    assert payload["downgrade_reason"] == "process_status_requires_manual_review"
    assert payload["manual_review_required"] is True


def test_readiness_proof_reports_running_health_as_unverified(
    tmp_path: Path,
    monkeypatch,
) -> None:
    runtime_root = tmp_path / "run"
    ...
    payload = build_readiness_proof_from_status_report(status_report).public_metadata()

    assert payload["readiness_status"] == ResidentDaemonReadinessProofStatus.READINESS_UNVERIFIED.value
...
def test_readiness_proof_reports_running_without_health_as_degraded(
    tmp_path: Path,
    monkeypatch,
) -> None:
    runtime_root = tmp_path / "run"
    ...
    assert payload["readiness_status"] == ResidentDaemonReadinessProofStatus.READINESS_DEGRADED.value
    assert payload["downgrade_reason"] == "process_existence_without_authenticated_readiness"
    assert payload["evidence_ceiling"] == "process_existence_without_authenticated_readiness"
