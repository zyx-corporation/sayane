...
def load_judge_config(level: int) -> JudgeConfig | None:
    """Load judge settings from ~/.omomuki/judge.yaml and environment."""
    if level < 2:
        return None

    home = Path.home() / ".omomuki"
    file_data: dict = {}
    ...
    base_url = os.environ.get("OMOMUKI_JUDGE_BASE_URL") or file_data.get("base_url")
    api_key = os.environ.get("OMOMUKI_JUDGE_API_KEY") or file_data.get("api_key")
    model = os.environ.get("OMOMUKI_JUDGE_MODEL") or file_data.get("model") or _DEFAULT_MODEL
