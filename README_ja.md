...
**現行リポジトリ版**: Community Edition **v1.0.14.post1**

**PyPI 公開版**: Community Edition **v1.0.14.post1**
...
| エディタや外部ツールから使う | CLI / Local Bridge / MCP Server / native macOS app / Chrome Extension（legacy） |
...
```bash
SAYANE_REF=v1.0.14.post1 curl -fsSL https://raw.githubusercontent.com/zyx-corporation/sayane/main/scripts/install.sh | bash
```
...
### E. native macOS app を開く（primary）

現行の primary operator-facing path は Bridge-hosted local shell ではなく、**native macOS app** です。
...
- native app から clipboard capture / candidate review / daemon guidance を routine path として使う
- Bridge 切断時は app 内の `Bridge を起動` / `接続を復旧` を使う
- `/app/ui` は debug-only compatibility surface として残る

詳しくは [Getting Started](docs/getting-started.md) と [`macos/SayaneApp/README.md`](macos/SayaneApp/README.md) を参照してください。

### F. resident app debug shell を開く（debug-only compatibility）

Bridge-hosted local shell は debug / fallback / handoff / browser-local smoke 向けです。
...
Community Edition **v1.0.14.post1** では、以下が利用可能です。
