...
### 1.0 Extension の権限（Chrome 日本語 UI）

| 表示 | 実際の意味 |
|------|------------|
| **閲覧履歴の読み取り**（旧版） | 多くの場合 `tabs` 権限の誤解を招く表示。**履歴 API は使わない** |
| **現在のサイトへのアクセス** 等 | `activeTab` — ツールバーから Extension を開いた**そのタブ**だけ |
| Bridge / ChatGPT / Claude | `host_permissions` で列挙したホストのみ |

manifest 変更後は「新しい権限」ダイアログが出ないことがある。**拡張機能を一度削除 → `extension/` を再読み込み**すると表示が更新される。
...
| OPT-03 | **Test connection** をクリック（token 空） | `Bearer token not configured`（赤） | ☐ | `/health` のみでは Pass しない |
| OPT-03b | token 入力後 **Test connection** | `Bridge OK (/health + /profiles)` | ☐ | 失敗時は PRE-01・token を再確認 |
...
**テストページ**: `https://example.com` など通常の **https://** ページ（`chrome://` / 新規タブ / PDF / Web Store は不可）。

**CAP の手順**: 対象タブを表示した状態で **Extension アイコンをクリック** → テキストを選択 → **Capture selection**（`activeTab` は「アイコンを押したときのアクティブタブ」にのみ有効）。
...
| ID | 手順 | 期待結果 | 結果 | 備考 |
|----|------|----------|------|------|
| CAP-01 | 上記テキストを選択 → **Capture selection** | ステータス `Captured: <uuid>` | ☐ | `Receiving end does not exist` → ページ種別 or タブ再読み込み |
...
**手順**: ChatGPT タブを前面にした状態で Extension アイコンをクリック → popup から Insert（`activeTab` + `host_permissions`）。

| ID | 手順 | 期待結果 | 結果 | 備考 |
|----|------|----------|------|------|
| INS-CG-01 | ChatGPT の**新規チャット**を開き、入力欄が表示されている | — | ☐ | |
| INS-CG-02 | popup → **Insert context (ChatGPT)** | ステータス `Inserted (chatgpt)` | ☐ | `Receiving end…` → Extension 更新後、**ChatGPT タブで**アイコンから開き直す |
...
| 症状 | 確認コマンド / 対処 |
|------|---------------------|
| Options の Save / Test が無反応 | `cd extension && npm run build` 後、Chrome で拡張機能を**更新**。0.5.5 以前は Options のボタン型バグあり |
| Bridge unreachable | `omomuki serve`、Options URL |
| `Receiving end does not exist`（CAP-01） | `https://` の通常ページ、`chrome://` 不可。Extension 更新後はタブ **F5** 再読み込み |
...
| Insert で `Receiving end does not exist` | **ChatGPT/Claude のタブ**で Extension を開く。`npm run build` 後に拡張機能を更新 |
| Insert 失敗 `INPUT_NOT_FOUND` | DOM 変更。`extension/src/sites/` のセレクタ更新 |
