...
- [macOS Native App](../macos/SayaneApp/README.md) — current primary operator-facing app path on macOS
...
- [CLI マニュアル `vault`](cli-manual.md) — Local Vault runtime / macOS keychain / unlock policy / schema diagnostics
...
- [Linux systemd --user unit 化](linux-systemd-user.md) — Linux で resident daemon の recorded shipped slice（追加実装は macOS 完了後に再開）
...
- [v1.0.76 curl+bash Operator Quickstart](release/v1.0.76-curl-bash-operator-quickstart.md) — 最短の導入・起動・確認手順
...
Sayane は、まずCLIで最小価値を実証し、その後 Local Bridge、MCP Server、native macOS app、Chrome Extension へ段階的に展開した。現行方針では **`curl | bash` による CLI / Bridge install** を主導線とし、その上で CLI / Bridge / MCP / native macOS app を利用する。Chrome Extension は freeze / deprecated である。直近は macOS app の完成を優先し、Linux / Windows の追加実装は記録済み状態で保留している。
