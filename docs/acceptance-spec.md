...
| CORE-05 | Profile 定義の `canonical_terms` をマージ・適用し、Adapter が正規語彙付き出力（Gemini/DeepSeek は `*_working_memo`）を返す。Core/テストは中立フィクスチャ（ABC 等）のみ使用 | `tests/test_canonical_terms.py`, [canonical-terminology.md](canonical-terminology.md) |
...
| Canonical terms | `test_canonical_terms.py` | CORE-05 |
