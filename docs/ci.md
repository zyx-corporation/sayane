...
pytest -v tests/test_sqlite_vault_store.py
...
- SQLite-backed VaultStore が plaintext default を返すこと
- SQLite-backed VaultStore が plaintext content を `encrypted_records` に保存すること
- 明示 test-only SQLite runtime builder が production default として扱われること
...
明示的な SQLite-backed test-only runtime 診断は以下で行う。

```bash
sayane vault status --test --sqlite path/to/vault.sqlite --json
```

`--sqlite` は `--test` なしでは拒否されなければならない。SQLite-backed 診断出力は `test_only: true`、`sqlite_backed: true`、`production_ready: false`、`sqlite_schema_errors: []` を含む必要がある。この経路は production backend ではない。
...
- SQLite-backed VaultStore は `is_plaintext_default() == False` を維持し、`encrypted_records` に plaintext content を保存してはならない
- 明示 test-only SQLite runtime builder は production default にしてはならない
...
- `sayane vault status --test --sqlite` は明示 test-only SQLite runtime 診断であり、production backend として扱ってはならない
