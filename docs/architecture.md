...
Chrome Extension は **入口と出口** である。同時に、LLM チャット UI と並ぶ **実行基盤上の実行要素** である。ここをおろそかにすれば、Capture・判断・Insert における入出力の意味が失われる（[ui-design-principles.md](./ui-design-principles.md)）。
...
**Authorization Layer（逸脱防止）**: 評価結果を客観的事実として扱わない、利用者許可を十分条件としない、self-eval を advisory に限定する。詳細は [authorization-layer.md](./authorization-layer.md) と [rde-audit-policy.md](./rde-audit-policy.md)。
...
CLI       : 信頼できる制御面
Bridge    : ローカル連携面
Extension : 実行要素としての対話 UI（capture / 判断 / insert）— [UI 設計](ui-design-principles.md)
Adapter   : LLM ランタイム固有形式への変換（チャット UI 側の消費）
```

Extension は Profile の本体ではないが、**人間↔Sayane の入出力境界**としては LLM チャットと同格に重要である。UI の劣化は文脈の意味喪失に直結する。
