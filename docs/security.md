# Omomuki Security Design

## 1. 目的

Omomuki は、ユーザーの人格的文脈・価値観・応答様式・作業方針を扱う。そのため、通常のローカルツール以上に privacy と security を重視する必要がある。
...
Webページや会話ログに、AIまたはOmomuki処理を誘導する命令が含まれる可能性がある。
...
Omomuki ProfileやContextにAPI key、password、token、private keyなどが混入する可能性がある。
...
Commercial Edition では **機密データ基準・許容範囲の登録** と **逸脱監査** を omomuki-pro で提供する（[confidentiality-audit.md](https://github.com/zyx-corporation/omomuki-pro/blob/main/docs/confidentiality-audit.md)）。RDE（意味変化）と相互補完する。
...
Omomukiにおいては、セキュリティ侵害は同時に意味変化の侵害でもある。
...
Commercial Edition では、機密区分・セクション許容範囲の登録と逸脱監査を [omomuki-pro](https://github.com/zyx-corporation/omomuki-pro/blob/main/docs/confidentiality-audit.md) で行う。
