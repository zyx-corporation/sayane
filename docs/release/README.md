...
- [ADR 0019 External Compatibility Artifacts Carry Retention and Redaction Metadata](../adr/0019-external-compatibility-artifacts-carry-retention-and-redaction-metadata.md) — accepted decision that compatibility exports carry redacted-path and retention guidance metadata
- [ADR 0023 Supported and Forbidden External Workflows for Vault-Aware Packages](../adr/0023-supported-and-forbidden-external-workflows-for-vault-aware-packages.md) — accepted decision that supported review packages declare allowed operator uses and forbidden workflows directly in manifest metadata
- [ADR 0024 Preview-Only `import-package` Contract and Permanent Legacy External Workflows](../adr/0024-preview-only-import-package-contract-and-permanent-legacy-external-workflows.md) — accepted decision that `import-package` stays non-mutating and that Obsidian/Git compatibility paths remain permanent legacy workflows
- [ADR 0025 Retention Expiry Remains Warning-Only and Sync Automation Stays Forbidden](../adr/0025-retention-expiry-remains-warning-only-and-sync-automation-stays-forbidden.md) — accepted decision that stale packages warn rather than hard-fail and that automatic sync paths remain outside the supported Community contract
- [ADR 0026 Explicit Review-Queue Import Command for Vault-Aware Packages](../adr/0026-explicit-review-queue-import-command-for-vault-aware-packages.md) — accepted decision that pending candidate persistence from packages uses a separate explicit command instead of changing `import-package`
- [ADR 0020 External Compatibility Export Targets Must Stay in Bounded Subdirectories](../adr/0020-external-compatibility-export-targets-must-stay-in-bounded-subdirectories.md) — accepted decision that compatibility exports may write only into safe bounded subdirectories under the chosen external vault root
- [ADR 0021 External Compatibility Imports May Be Bounded to Safe Subdirectories](../adr/0021-external-compatibility-imports-may-be-bounded-to-safe-subdirectories.md) — accepted decision that compatibility imports may be limited to one safe bounded subtree under the external vault root
...
- [v1.0.69 P2 Working-Store Boundary Closure Release Note](v1.0.69-p2-working-store-boundary-closure-release-note.md) — closes the remaining P2 boundary by fixing FileSystem working stores to debug / legacy / import-fixture compatibility only
- [v1.0.70 P3 External Integration Explicit Legacy Opt-In Release Note](v1.0.70-p3-external-integration-explicit-legacy-opt-in-release-note.md) — starts P3 by turning Obsidian import/export and Git commit into explicit compatibility workflows
- [v1.0.71 P3 Vault-Aware External Package Preview Release Note](v1.0.71-p3-vault-aware-external-package-preview-release-note.md) — adds the first supported reviewable external package flow distinct from legacy compatibility workflows
- [v1.0.72 P3 Explicit Review-Queue Import Release Note](v1.0.72-p3-explicit-review-queue-import-release-note.md) — closes the remaining P3 package-import gap with an explicit pending-candidate queue command
