...
| 項目 | 説明 |
|------|------|
| 設定ファイル | `~/.omomuki/judge.yaml` |
| 環境変数 | 同名キーは **環境変数が優先**（ファイルより上書き） |
| 例ファイル | [examples/judge.yaml.example](../examples/judge.yaml.example) |

| キー / 環境変数 | 説明 |
|----------------|------|
| `base_url` / `OMOMUKI_JUDGE_BASE_URL` | OpenAI 互換 API のベース URL（末尾 `/v1`） |
| `api_key` / `OMOMUKI_JUDGE_API_KEY` | Bearer トークン（Level 3 では必須） |
| `model` / `OMOMUKI_JUDGE_MODEL` | モデル名（未指定時 `llama3.2`） |
...
`api_key` を YAML に書くこともできるが、**リポジトリや共有ディレクトリへ置かない**こと。可能なら `OMOMUKI_JUDGE_API_KEY` のみ環境変数で渡す。
...
```bash
export OMOMUKI_JUDGE_BASE_URL="https://api.openai.com/v1"
export OMOMUKI_JUDGE_API_KEY="sk-..."          # 実際のキーに置き換え
export OMOMUKI_JUDGE_MODEL="gpt-4o-mini"
```
...
前提: Omomuki インストール済み、[Ollama](https://ollama.com/) インストール済み。
...
```bash
mkdir -p ~/.omomuki
cp examples/judge.yaml.example ~/.omomuki/judge.yaml
...
```bash
omomuki candidate list
omomuki candidate evaluate <candidate-id> --level 2
```

成功時、`evaluation.llm_review` にモデル名・LLM 側 RDE 分類・notes が入る。

```bash
omomuki candidate show <candidate-id>
```
...
```bash
omomuki candidate diff <candidate-id>
omomuki candidate approve <candidate-id>
omomuki candidate lineage --profile-id default
```
...
```bash
export OMOMUKI_JUDGE_BASE_URL="https://api.openai.com/v1"
export OMOMUKI_JUDGE_API_KEY="sk-..."
export OMOMUKI_JUDGE_MODEL="gpt-4o-mini"
```
...
```bash
omomuki candidate evaluate <candidate-id> --level 3
```
...
Omomuki **0.5.8**（Bridge/MCP candidate API 含む）
