...
Omomuki **0.5.0** 時点で、Phase 0〜5 の成果物が利用可能である。本書は「何ができて」「どこから始めるか」をまとめる。
...
**未実装（Phase 6 以降）**: LLM-as-a-Judge（Level 2+）、Rust 抽出など。
...
```bash
omomuki storage import /path/to/obsidian-vault
...
- LLM-as-a-Judge（RDE Level 2+）は未実装
