...
**現行バージョン**: Community Edition **v1.0.3**（PyPI: `pip install sayane` · Phase 0〜5）
...
Community Edition では **chatgpt** / **claude** / **gemini** / **deepseek** / **local-openwebui** などに対応しています。
...
特定バージョン（例: v1.0.3）をスクリプトで入れる場合:

```bash
SAYANE_REF=v1.0.3 curl -fsSL https://raw.githubusercontent.com/zyx-corporation/sayane/main/scripts/install.sh | bash
```
...
- Adapter **target**: chatgpt / claude / gemini / deepseek / local-openwebui（Extension DOM insert は別途ビルド要）
- PyPI: `pip install sayane`（CLI + Bridge + MCP）。Extension は別途 [install.md](docs/install.md)
