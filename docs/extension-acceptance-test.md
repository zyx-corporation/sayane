...
| 項目 | 値 |
|------|-----|
| **Sayane** | **0.5.8** 以降（`sayane --version`） |
| **Extension manifest** | **0.3.8** 以降 |
| **所要時間** | 30〜45 分（Level 2 評価を含む場合は +10 分） |

関連: [Extension マニュアル](extension-manual.md) / [Dogfood 手順書](dogfood-walkthrough.md) / [OSS 版受け入れテスト仕様](acceptance-spec.md) · [#89](https://github.com/zyx-corporation/sayane/issues/89) L3 UAT · [#93](https://github.com/zyx-corporation/sayane/issues/93) [手動のみ](acceptance-manual-only.md)

### 実装メモ（0.5.8 / Extension 0.3.3）
...
| Options **表示言語**（i18n） | `displayLanguage`: auto / en / ja（`locale/en.json`, `locale/ja.json`） |
...
**UI 状態表示**（設計の基本コンセプト）: [ui-design-principles.md](ui-design-principles.md)。Evaluate 実行中は砂時計カーソル（`wait`）、Bridge 未接続で Capture が押せないときは禁止カーソル（`not-allowed`）＋理由ヒントであること。
...
| Sayane | `pip install -e ".[dev]"` 済み、`sayane --version` → `sayane 0.5.8` |
| Bridge | `sayane serve` を**別ターミナルで常駐** |
| ネットワーク | ローカルのみ（`127.0.0.1:38741`） |
| 任意（L2） | Ollama 起動、`SAYANE_JUDGE_MODEL` 設定済み |

### 1.0 Extension の権限（Chrome 日本語 UI・0.3.2）
...
| PRE-02 | `~/.sayane/bridge.token` が存在する | ファイルあり | | |
| PRE-03 | `sayane profile inspect` | default 等の Profile 表示 | | |
| PRE-04 | `extension/dist/` がビルド済み | `npm run build` 成功後 | | |
| PRE-05 | Extension が Chrome に読み込まれている | 拡張機能一覧に Sayane **0.3.3** | | |
...
| ID | 手順 | 期待結果 | 結果 | 実測・メモ |
|----|------|----------|:----:|------------|
| POP-01 | Extension アイコン → popup | 「Sayane」、ステータス行 | | |
...
```text
Sayane acceptance test capture — explicit uncertainty and local-first portability.
```
...
| CND-L2-03 | `sayane candidate show <id>` | `llm_review` 非 null（成功時） | | |
...
# Chrome → Sayane → 更新（manifest 変更時は削除→再読み込み）
```
...
| `Bridge unreachable` | `sayane serve` |
...
| Approve 400 | RDE class 確認、`sayane candidate diff` |
...
| 日付 | 版 | 内容 |
|------|-----|------|
| 2026-05-23 | 0.5.8 / Ext 0.3.2 | i18n（OPT-05）、版番号同期 |
