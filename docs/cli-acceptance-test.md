...
| 項目 | 値 |
|------|-----|
| **Sayane** | **1.0.0+**（`sayane --version`） |
| **対象** | Community Edition（`filesystem` backend、sayane-pro 未インストール） |
| **所要時間** | L1: CI。L2 手動レビュー 20〜40 分（[#92](https://github.com/zyx-corporation/sayane/issues/92) 以降、多くの ✓ は L1 のみ） |
| **上位仕様** | [acceptance-spec.md](acceptance-spec.md) L2 の CLI 部分 |
| **関連 Issue** | [#88](https://github.com/zyx-corporation/sayane/issues/88) L2 Core · [#93](https://github.com/zyx-corporation/sayane/issues/93) [手動のみ](acceptance-manual-only.md) |
...
**考察の結論**: pytest は**回帰の安全網**。Candidate フロー・Critical 拒否・ERR 経路・Storage export/commit は [#92](https://github.com/zyx-corporation/sayane/issues/92) で L1 化済み。L2-CLI で残るのは [acceptance-manual-only.md §4](acceptance-manual-only.md)（Markdown 読みやすさ、実 vault 目視など）。
...
cd /path/to/sayane
git pull
uv run --extra dev sayane --version   # 1.0.0+
uv run --extra dev pytest -q          # PRE-C01
# 明示的な local venv を使う場合:
# python -m venv .venv && source .venv/bin/activate
# pip install -e ".[dev]"
# sayane --version          # 1.0.0+
# pytest -q                 # PRE-C01
```
...
| PRE-C02 | `sayane --version` | `sayane 1.0.0` 以上 |
...
| 区分 | ID |
|------|-----|
| **L1（CI）** | §1.4 の L1 根拠列 — [#92](https://github.com/zyx-corporation/sayane/issues/92) で `test_acceptance_cli.py` 等に集約 |
| **L2 手動必須** | [acceptance-manual-only.md §4](acceptance-manual-only.md)（EXP-01, I18N-01, STOR-04〜05, COMP-03 等） |
| **L2 記録（acceptance-spec）** | PRE-M01, SERVE-M01, STOR-M02 — CLI-M01〜03 は **L1 のみ**（再実施不要） |
| **任意** | CAND-08, `--force-critical` 成功確認 |
| **Fail** | L2 手動必須 NG → Issue 起票 → 修正 → 再実施 |

リリース時: **CI green** + acceptance-spec §4.0 の **L2 必須列** + Extension 変更時は L3。
