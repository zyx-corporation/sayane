...
この installer は通常 install で PyPI を優先し、必要時だけ git fallback する。

ローカルで installer の主導線を確認する場合:
...
現時点では **macOS を先に完成させる** 方針で進めており、Linux / Windows の追加実装は
現在の contract / shipped slice を記録した状態で保留している。

native app 上では、`Home` / `Bridge Status` / `Daemon` / compact `Error` view から同じ
起動導線を辿れる。local script path が見えている面では `Open Launcher` と
`Copy Startup Command` を使う。debug compatibility path は routine card には出さず、
必要時だけ diagnostics から `Open Compatibility Shell` と `Copy Compatibility Shell URL`
を使う。これらの debug 操作は、
...
### 5.6 macOS native app launch / smoke
...
現在画面のまま回復完了を判断しやすい。
進行中は caution banner、失敗時は Terminal window と `~/.sayane/run-app-local.log`
を見る案内が同じバナーに出る。
同じ回復操作の多重実行を避けるため、進行中は主要な recovery button も一時的に無効化される。
その間、ボタン文言も `Bridge 起動中…` / `再接続中…` / `更新中…` に切り替わる。
さらに Bridge 状態パネルと error card には spinner も出て、open launcher /
open compatibility shell は回復完了まで一時停止する。
上部の結果バナーも進行中メッセージでは spinner を併記する。
toolbar の `Refresh` も同じ状態に追従し、回復中は `更新中…` / `再接続中…` などへ切り替わって無効化される。
...
さらに Compatibility Shell と最初の Next Command / Reason も冒頭に入るため、ブラウザ側の互換確認と
daemon 側の初手判断を同じ exported note からすぐ始められる。token file が読める環境では、
この compatibility shell entry も bootstrap 優先の URL で書き出される。
...
### 5.7 Obsidian / Git（Phase 5 / compatibility）
